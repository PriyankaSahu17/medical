...
const Appointments = () => {
  ...
  useEffect(() => {
    const fetchUserNotifications = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/auth/${userId}/notificationvs`);
  ...
  const handleAccept = async (notificationId,notificationphone) => {
    setAcceptedVolunteer(userData);
    sendsms(notificationphone);
  };
  const sendsms = async (notificationphone) => {

      const message = `Volunteer ${acceptedVolunteer.name} has accepted the request. Contact: ${acceptedVolunteer.phone}`;
      const phoneNumber = notificationphone;
      const response = await axios.post(`${BASE_URL}/sms/send-sms`, {
        phoneNumber,
        message,
      });
